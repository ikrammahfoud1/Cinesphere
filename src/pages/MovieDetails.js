...
import useMovieDetails from '../hooks/useMovieDetails';  // Importer le hook

const MovieDetails = () => {
  const { id } = useParams();  // Récupérer l'ID du film ou de la série depuis l'URL
  const { details, video, loading, error } = useMovieDetails(id);  // Utiliser le hook pour récupérer les détails
